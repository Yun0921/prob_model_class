...
// 計算高斯分布pdf (已在程式中有GSLfun_ran_gaussian_pdf)
double gaussian_pdf(double x, double mu, double sigma) {
   double diff = x - mu;
   return (1.0 / (sigma * sqrt(2 * M_PI))) * exp(-diff * diff / (2 * sigma * sigma));
}

// EM 演算法求2成分GMM的最大似然估計
Gauss_mixture_params EM_GaussMixture_MLE(double *data, int N, int max_iter, double tol) {
   Gauss_mixture_params params;

   // 初始化參數：混合比例 0.5，均值隨機取樣本均值，sigma取樣本標準差
   params.mixCof = 0.5;
   double mean = 0.0;
   ...
   var /= N;
   params.Gauss1.mu = mean - 0.5;
   params.Gauss1.sigma = sqrt(var);
   params.Gauss2.mu = mean + 0.5;
   params.Gauss2.sigma = sqrt(var);

   double prev_log_likelihood = -INFINITY;
   for(int iter=0; iter < max_iter; iter++) {
       // E-step: 計算每筆資料屬於成分1的機率
       double *resp = malloc(sizeof(double)*N);
       double log_likelihood = 0.0;

       for(int i=0; i<N; i++) {
           double p1 = params.mixCof * gaussian_pdf(data[i], params.Gauss1.mu, params.Gauss1.sigma);
           double p2 = (1 - params.mixCof) * gaussian_pdf(data[i], params.Gauss2.mu, params.Gauss2.sigma);
           double denom = p1 + p2;
           resp[i] = p1 / denom;
       ...
       // M-step: 根據resp更新參數
       double sum_resp = 0.0;
       double sum_resp_x = 0.0;
       double sum_resp_x2 = 0.0;
       for(int i=0; i<N; i++) {
           sum_resp += resp[i];
           sum_resp_x += resp[i] * data[i];
           sum_resp_x2 += resp[i] * data[i] * data[i];
       }
       params.Gauss1.mu = sum_resp_x / sum_resp;
       double var1 = (sum_resp_x2 / sum_resp) - (params.Gauss1.mu * params.Gauss1.mu);
       params.Gauss1.sigma = sqrt(var1 > 1e-6 ? var1 : 1e-6);
       ...
       for(int i=0; i<N; i++) {
           double r2 = 1 - resp[i];
           sum_resp += r2;
           sum_resp_x += r2 * data[i];
           sum_resp_x2 += r2 * data[i] * data[i];
       }
       params.Gauss2.mu = sum_resp_x / sum_resp;
       double var2 = (sum_resp_x2 / sum_resp) - (params.Gauss2.mu * params.Gauss2.mu);
       params.Gauss2.sigma = sqrt(var2 > 1e-6 ? var2 : 1e-6);

       // 更新混合比例
       double sum_resp_total = 0.0;
       for(int i=0; i<N; i++) sum_resp_total += resp[i];
       params.mixCof = sum_resp_total / N;
...
// 計算混合模型在資料上的最大似然值
double mixture_max_likelihood(double *data, int N, Gauss_mixture_params params) {
   double lik = 1.0;
   for(int i=0; i<N; i++) {
       double p = params.mixCof * gaussian_pdf(data[i], params.Gauss1.mu, params.Gauss1.sigma)
                + (1 - params.mixCof) * gaussian_pdf(data[i], params.Gauss2.mu, params.Gauss2.sigma);
       lik *= p;
   }
   return lik;
}
...
int main( int argc, char *argv[] ){
  ...
  for(  uint iter= 0;  iter < datasets_n;  ++iter  ){
    ...
    Gauss_mixture_params mix_params = EM_GaussMixture_MLE(data, dataN, 1000, 1e-6);
    double gauss1_max_lik = data_Gauss1_maxLikelihood();
    double max_lik = mixture_max_likelihood(data, dataN, mix_params);
    printf("1-component maximum likelihood: %g\n", gauss1_max_lik);
    printf("2-component mixture maximum likelihood: %g\n", max_lik);
